...
local decodeBase64 = crypt and crypt.base64 and crypt.base64.decode or function(data) 
...
local assets = {}
local cacheFolder = "lv.vila/cache/"
    ...
    local success, err = xpcall(function()
        ...
        if isFile(path) then
            ...
            if customAsset and customAsset ~= "" then
                assets[id] = customAsset
            end
        end
    end, function(e)
        warn("[lv.vila] Failed to load asset", id, ":", e)
    end)
...
-- Load all assets with small delay
for id in pairs(assetData) do
    safeLoadAsset(id)
    task.wait()
end

-- FIXED: Safe getAsset with multiple fallback layers
local function getAsset(id)
    ...
    -- Try Roblox fallback assets
    local fallbacks = {
        square = "rbxasset://textures/ui/ScrollBarVerticalBackground.png",
        checkmark = "rbxassetid://6031097227",
        triangle = "rbxassetid://6031094680",
...
function Window.new(title, position, size)
    ...
    if self.keybindPopup[1] then
        self.toggleButton = self:addObject("TextButton", {
            Text = "Toggle",
        ...
        self.holdButton = self:addObject("TextButton", {
            Text = "Hold",
            TextColor3 = Color3.new(1, 1, 1),
            Font = Enum.Font.Gotham,
            TextSize = 12,
